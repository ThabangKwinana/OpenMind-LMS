            ...
            try
            {
                ...
                if (assignNum == 1)
                {
                    if (int.TryParse(assignMarkTxtBox.Text.Trim(), out assignMark))
                    {
                        ...
                        {
                            string sql = $"UPDATE MathsGbTB SET Ass1_Mark = '{assignMark}', Teacher_ID = '{teacherIdTxtBox.Text.Trim()}' WHERE Learner_ID = '{learnerIdTxtBox.Text.Trim()}'";
                            update(sql);
                        }
                        else
                        {
                    ...
                    {
                        Response.Write("<script>alert('Assignment Mark is invalid. it must be numeric');</script>");
                    }
                }
                ...
                if (assignNum == 2)
                {
                    if (int.TryParse(assignMarkTxtBox.Text.Trim(), out assignMark))
                    {
                        if (assignMark < 100 && assignMark > 0)
                        {
                            string sql = $"UPDATE MathsGbTB SET Ass2_Mark = '{assignMark}', Teacher_ID = '{teacherIdTxtBox.Text.Trim()}' WHERE Learner_ID = '{learnerIdTxtBox.Text.Trim()}'";
                            update(sql);
                        }
                        else
                        {
                ...
                if (assignNum == 3)
                {
                    if (int.TryParse(assignMarkTxtBox.Text.Trim(), out assignMark))
                    {
                        if (assignMark < 100 && assignMark > 0)
                        {
                            string sql = $"UPDATE MathsGbTB SET Ass3_Mark = '{assignMark}', Teacher_ID = '{teacherIdTxtBox.Text.Trim()}' WHERE Learner_ID = '{learnerIdTxtBox.Text.Trim()}'";
                            update(sql);
                        }
                        else
                        {
                ...
                if (testNum == 1)
                {
                    if (int.TryParse(testMarkTextBox.Text.Trim(), out testMark))
                    {
                        ...
                        {
                            string sql = $"UPDATE MathsGbTB SET Test1_Mark = '{testMark}', Teacher_ID = '{teacherIdTxtBox.Text.Trim()}' WHERE Learner_ID = '{learnerIdTxtBox.Text.Trim()}'";
                            update(sql);
                        }
                        else
                        {
                ...
                if (testNum == 2)
                {
                    if (int.TryParse(testMarkTextBox.Text.Trim(), out testMark))
                    {
                        if (testMark < 100 && testMark > 0)
                        {
                            string sql = $"UPDATE MathsGbTB SET Test2_Mark = '{testMark}', Teacher_ID = '{teacherIdTxtBox.Text.Trim()}' WHERE Learner_ID = '{learnerIdTxtBox.Text.Trim()}'";
                            update(sql);
                        }
                        else
                        {
                ...
                if (testNum == 3)
                {
                    if (int.TryParse(testMarkTextBox.Text.Trim(), out testMark))
                    {
                        if (testMark < 100 && testMark > 0)
                        {
                            string sql = $"UPDATE MathsGbTB SET Test3_Mark = '{testMark}', Teacher_ID = '{teacherIdTxtBox.Text.Trim()}' WHERE Learner_ID = '{learnerIdTxtBox.Text.Trim()}'";
                            update(sql);
                        }
                        else
                        {
