...
namespace OpenMind
{
    public partial class EngGbPage : System.Web.UI.Page
    {
        ...
        protected void requestMarkBtn_Click(object sender, EventArgs e)
        {
            ...
            if (type == "Tests")
            {
                string sql = $"SELECT Learner_ID, Test1_Mark, Test2_Mark, Test3_Mark FROM EnglishGbTB WHERE Learner_ID = '{learnerIDTxtBox.Text.Trim()}'";
                display(sql);
            }

            if (type == "Assignments")
            {
                string sql = $"SELECT Learner_ID, Ass1_Mark, Ass2_Mark, Ass3_Mark FROM EnglishGbTB WHERE Learner_ID = '{learnerIDTxtBox.Text.Trim()}'";
                display(sql);
            }
        }
        ...
        public void display(string query)
        {
            try
            {
                ...
                EnglishGridView.DataSource = ds;
                EnglishGridView.DataBind();
