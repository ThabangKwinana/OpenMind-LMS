...
namespace OpenMind
{
    public partial class registerAdmin : System.Web.UI.Page
    {
        ...
        protected void regAdminBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string id = adminIdTextBx.Text.Trim();
                string name = adminFirstNameTxtBox.Text.Trim();
                string lastName = adminLastNameTxtBox.Text.Trim();
                    ...
                    else
                    {
                        ...
                        if (match == false)
                        {
                            try
                            {
                                ...
                                string sql = $"INSERT INTO AdminTB(AdminID, Password, Fname, Lname, ID_Num) VALUES('{idCode}', '{passwordTxtBox.Text}', '{name}', '{lastName}', '{id}')";
        ...
        private void addToDb(string query)
        {
            try
            {
                ...
                Response.Write("<script>alert('Admin Registrated Successfully');</script>");
                Response.Write("<script>alert('" + idCode + "');</script>");
                adminFirstNameTxtBox.Text = "";
                adminLastNameTxtBox.Text = "";
                adminIdTextBx.Text = "";
            }
            catch (SqlException ex)
            {
