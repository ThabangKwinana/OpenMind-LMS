...
namespace OpenMind
{
    public partial class deletePage : System.Web.UI.Page
    {
        ...
        protected void dltLearnBtn_Click(object sender, EventArgs e)
        {
            try
            {
                string sql = $"DELETE FROM PhysicsGbTB  WHERE Learner_ID = '{learnerIDTxtBox.Text.Trim()}' ";
        ...
        private void delLearnMarks(string query)
        {
            try
            {
                ...
                Response.Write("<script>alert('Learner Marks Record Deleted Successfully');</script>");
