...
namespace OpenMind
{
    public partial class deleteTeachers : System.Web.UI.Page
    {
        ...
        protected void dltTcrBtn_Click(object sender, EventArgs e)
        {
            try
            {

                string sql = $"DELETE FROM TeacherTB  WHERE Teacher_ID = '{teacherIDTxtBox.Text.Trim()}' ";
        ...
        private void delLnrDetails(string query)
        {
            try
            {
                ...
                Response.Write("<script>alert('Teacher Details Deleted Successfully');</script>");
